...
export const ViewSingleTask = ({ theme, viewSingle, setViewSingle }: TaskType) => {
  ...
  return (
    <article 
      className={`absolute left-1 top-16 h-[83%] transition-all grid place-content-center shadow-xl ${viewSingle == 'Open' ? 'scale-100' : 'scale-0'} ${theme == 'light' ? 'bg-slate-500 bg-opacity-40' : 'bg-slate-900 bg-opacity-60 shadow-slate-700'} w-[98%] z-10  rounded-md`}>
      <div className={`relative flex flex-col z-50 h-44 w-[340px] bg-red-600 rounded-md ${theme == 'light' ? 'bg-slate-200' : 'bg-slate-700'}`}>
        ...
        <div 
          className="taskbars text-justify text-[12px] whitespace-pre-wrap p-2 pt-0 overflow-y-scroll flex flex-col gap-1">
            {task?.task}
